...
export default function Home() {
  ...
  const [beforeClarity, afterClarity = ''] = afterComplex.split('清晰');
  ...
  return <main>
    <RestoreScroll path="/" />
    <SiteHeader />
<section className="hero" aria-labelledby="hero-title"><p className="eyebrow">{t('home.role', 'Staff Product Designer · UIUX Designer')}</p><h1 id="hero-title" className={language === 'zh' ? 'zh-display' : undefined}>{language === 'zh' ? <>{beforeComplex}<span className="hero-complex">複雜</span>{beforeClarity}<i className="hero-clarity">清晰</i>{afterClarity}</> : <><b className="hero-english-line">Shape <span className="hero-complex">complexity</span></b>{' '}<b className="hero-english-line">into <i className="hero-clarity">clarity.</i></b></>}</h1><p className="hero-copy">{t('home.intro', "Hey, I'm Kimberly Toh! I simplify complex workflows, design scalable systems, and turn business needs into intuitive product experiences.")}</p></section>
