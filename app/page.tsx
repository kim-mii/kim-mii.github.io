...
function ProjectVisual({ kind, children }: { kind: string; children?: React.ReactNode }) { return <div className={`project-visual project-visual--${kind}`}><i /><b /><em /><span />{children}</div>; }
...
export default function Home() {
  return <main>
    ...
    <section id="contributions" className="section contributions" aria-labelledby="contributions-title"><div className="section-intro"><p className="eyebrow">01 / Selected work</p><h2 id="contributions-title">My Work</h2></div><div className="project-grid">{[[0, 2], [1, 3]].map((column) => <div className="project-column" key={column.join('-')}>{column.map((index) => { const project = projects[index]; return <Link href={`/projects#${project.name.toLowerCase().replaceAll(' ', '-')}`} className={`project project--${index + 1}`} key={project.name} aria-label={`View ${project.name} project`}><ProjectVisual kind={project.kind}><span className="project-tag">{project.type}</span><div className="project-caption"><span>{String(index + 1).padStart(2, '0')}</span><div><h3>{project.name}</h3></div></div></ProjectVisual></Link>; })}</div>)}</div><Link className="soft-button all-works" href="/projects">Explore All Works</Link></section>
