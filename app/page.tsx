...
export default function Home() {
  ...
  const chineseHeroTitle = t('home.title', '從把故事說好，到把產品做好');
  const [beforeStory, afterStory = ''] = chineseHeroTitle.split('故事');
  const [betweenStoryAndProduct, afterProduct = ''] = afterStory.split('產品');
  ...
  return <main>
    <RestoreScroll path="/" />
    <SiteHeader />
<section className="hero" aria-labelledby="hero-title"><p className="eyebrow">{t('home.role', 'Staff Product Designer · UIUX Designer')}</p><h1 id="hero-title" className={language === 'zh' ? 'zh-display' : undefined}>{language === 'zh' ? <>{beforeStory}<i className="hero-clarity">故事</i>{betweenStoryAndProduct}<span className="hero-complex">產品</span>{afterProduct}</> : <><b className="hero-english-line">Shape <span className="hero-complex">complexity</span></b>{' '}<b className="hero-english-line">into <i className="hero-clarity">clarity.</i></b></>}</h1><p className="hero-copy">{t('home.intro', "Hey, I'm Kimberly Toh! I simplify complex workflows, design scalable systems, and turn business needs into intuitive product experiences.")}</p></section>
