...
export default function Home() {
  return <main>
    <header className="site-header"><Link className="wordmark" href="/">Kimberly Toh</Link><nav aria-label="Primary navigation"><Link href="/milestone">My Milestones</Link><Link href="/projects">My Contributions</Link><Link href="/about">Me</Link></nav></header>
    <section className="hero" aria-labelledby="hero-title"><p className="eyebrow">Staff Product Designer · UIUX Designer</p><h1 id="hero-title">I shape <span>complexity</span><br />into <i>clarity.</i></h1><p className="hero-copy">I simplify complex workflows, design scalable systems, and turn business needs into intuitive product experiences.</p><a className="quiet-link" href="#contributions">Explore selected work <b>↓</b></a></section>
    <section id="contributions" className="section contributions" aria-labelledby="contributions-title"><div className="section-intro"><p className="eyebrow">01 / Selected work</p><h2 id="contributions-title">My Contributions</h2></div><div className="project-grid">{projects.map((project, i) => <Link href={`/projects#${project.name.toLowerCase().replaceAll(' ', '-')}`} className={`project ${project.className}`} key={project.name} aria-label={`View ${project.name} project`}><ProjectVisual kind={project.kind} /><div className="project-caption"><span>{String(i + 1).padStart(2, '0')}</span><div><h3>{project.name}</h3><p>{project.type}</p></div><b>↗</b></div></Link>)}</div></section>
    <section className="section adventures" aria-labelledby="adventures-title"><div className="section-intro"><p className="eyebrow">02 / Experience</p><h2 id="adventures-title">My Adventures</h2></div><div className="experience-labels" aria-hidden="true"><span>Company</span><span>Role</span><span>Period</span></div><div className="experience-list">{experiences.map(([company, roles]) => <article className="experience" key={company as string}><h3>{company as string}</h3><div>{(roles as string[][]).map(([role, period]) => <div className="role" key={role}><p>{role}</p><time>{period}</time></div>)}</div></article>)}</div></section>
    <section className="section know-me" aria-labelledby="know-title"><div className="know-heading"><p className="eyebrow">03 / A little more</p><h2 id="know-title">Get to<br /><i>know me</i></h2><Link className="soft-button" href="/about">Get to know me <span>↗</span></Link></div><div className="know-content"><p className="statement">I’m at my best where systems have many moving parts and the right questions make the path forward feel unexpectedly simple.</p><div className="clusters"><div><p className="eyebrow">Ways of working</p><ul><li>Product Design</li><li>UX Design</li><li>UI Design</li><li>UX Copywriting</li><li>Design System</li><li>UX Research</li><li>Information Architecture</li><li>Design with AI</li></ul></div><div><p className="eyebrow">Tools</p><ul><li>Figma</li><li>Adobe Creative Suite</li><li>Maze</li></ul><p className="eyebrow language-label">Languages</p><ul><li>Chinese</li><li>English</li><li>Malay</li><li>Cantonese / Hokkien</li></ul></div></div></div></section>
    <section className="brand-section" aria-labelledby="brands-title"><div className="section-intro"><p className="eyebrow">04 / Collaboration</p><h2 id="brands-title">Built with teams that <i>move things forward</i></h2></div><div className="marquee" tabIndex={0} aria-label="Brands Kimberly has collaborated with. Focus to pause the carousel."><div className="marquee-track">{[...brands, ...brands].map(([brand, detail], index) => <button className="brand" key={`${brand}-${index}`} type="button" aria-label={`${brand}: ${detail}`}><span className="tooltip" role="tooltip">{detail}</span><strong>{brand}</strong></button>)}</div></div></section>
    <footer><p>Shape what<br /><i>matters</i></p><div className="footer-links"><a href="/Kimberly_2026履歷.pdf" target="_blank" rel="noreferrer">View Resume <span>↗</span></a><a href="https://www.linkedin.com/in/kimmy-yh/" target="_blank" rel="noreferrer">LinkedIn <span>↗</span></a><a href="mailto:kimmy.yh93@gmail.com">Drop a mail <span>↗</span></a></div><small>© 2026 Kimberly Toh</small></footer>
  </main>;
}
