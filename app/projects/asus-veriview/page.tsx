...
export default function AsusVeriView() {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver((entries) => entries.forEach((entry) => {
      if (entry.isIntersecting) setVisible((current) => current.includes(entry.target.dataset.reveal || '') ? current : [...current, entry.target.dataset.reveal || '']);
    }), { threshold: .01 });
  ...
  return <main className="case-page">
    ...
    <figure className={`${reveal('final')} case-final`} data-reveal="final"><img src="/assets/projects/asus-veriview/veriview-final-1.jpg" loading="lazy" alt="ASUS VeriView UX research, information architecture, wireframes, design system, and final UI" /></figure>
