...
const logos = [
  ...
  { name: 'Revenue Harvest', category: 'Web UI/UX Design', src: '/assets/collaboration/revenue-harvest-logo.png' },
    ...
    <div className="logo-carousel-track">
      {[false, true].map((clone) => <div className="logo-carousel-group" aria-hidden={clone || undefined} key={clone ? 'clone' : 'original'}>
        {logos.map(({ name, category, src, tone = '' }) => {
          ...
          return clone
            ? <div className={logoClass(tone, Boolean(src))} key={`${name}-clone`}>{src && <img src={src} alt="" />}<span className="logo-fallback" aria-hidden="true">{name}</span></div>
